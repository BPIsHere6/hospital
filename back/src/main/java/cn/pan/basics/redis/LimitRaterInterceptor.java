package cn.pan.basics.redis;

import cn.pan.basics.exception.PanException;
import cn.pan.basics.parameter.*;
...
import cn.pan.data.vo.HttpIpSsoSetting;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    public HttpIpSsoSetting getHttpIpSsoSetting(){
        ...
        if(s != null && !PanNullUtils.isNull(s.getValue())){
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,@ApiParam(name = "响应的处理器") Object handler) throws Exception {
        ...
        // 单IP限流判断
        if(panLoginProperties.getOneLimiting()) {
            boolean flag1 = redisRaterLimiter.getLimitFlag(ip,panLoginProperties.getOneLimitingSize(), panLoginProperties.getOneLimitingTime());
            if (!flag1) {
                throw new PanException("您的请求过于频繁，请稍后再试！");
            }
        }
        // 全局限流判断
        if(panLoginProperties.getAllLimiting()){
            boolean flag2 = redisRaterLimiter.getLimitFlag("pan_LIMIT_ALL",panLoginProperties.getAllLimitingSize(), panLoginProperties.getAllLimitingTime());
            if (!flag2) {
                throw new PanException("系统已达到最大承载量，无法继续提供服务，请稍后再试！");
            }
        }
        // IP黑名单
        HttpIpSsoSetting os = getHttpIpSsoSetting();
        if(os!=null && !PanNullUtils.isNull(os.getBlacklist())){
            String[] list = os.getBlacklist().split("\n");
            for(String item : list){
                if(Objects.equals(ip,item)) {
                    throw new PanException("您已被禁止访问该系统，如有疑问请咨询管理员，谢谢！");
                }
            }
        }
        // 特定方法限流判断
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (!flag3) {
                    throw new PanException(method.getName() + "方法请求超限，请稍后再试");
                }
            }
        }catch (Exception e){}
        // 放行至下一个拦截器
