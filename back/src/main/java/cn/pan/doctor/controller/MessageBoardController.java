...
@RestController
@Api(tags = "留言板管理")
@RequestMapping("/pan/messageBoard")
@Transactional
public class MessageBoardController {
    ...
    public Result<IPage<MessageBoard>> getByPage(@ModelAttribute MessageBoard board,@ModelAttribute PageVo page){
        ...
        if(!PanNullUtils.isNull(board.getDate())) {
            qw.eq("date",board.getDate());
        }
        if(PanNullUtils.isNull(board.getReplyId())) {
            qw.eq("reply_id","");
        }
        else {
    ...
    public Result<MessageBoard> insert(@RequestParam String content){
        if(PanNullUtils.isNull(content)) {
        ...
        messageBoard.setDate(getpanNowTime());
