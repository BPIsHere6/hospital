...
import cn.pan.doctor.entity.DoctorScheduling;
import cn.pan.doctor.service.IDoctorSchedulingService;
...
@RestController
@Api(tags = "医生排班管理")
@RequestMapping("/pan/doctorScheduling")
@Transactional
public class DoctorSchedulingController {
    ...
    public Result<IPage<DoctorScheduling>> getByPage(@ModelAttribute DoctorScheduling scheduling,@ModelAttribute PageVo page){
        ...
        if(!PanNullUtils.isNull(scheduling.getDoctorId())) {
            qw.eq("doctor_id",scheduling.getDoctorId());
        }
        if(!PanNullUtils.isNull(scheduling.getDate())) {
