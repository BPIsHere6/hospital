<template>
  <div class="search">
    ...
    <Modal
      v-model="addMessageModal"
      title="添加留言"
      ok-text="确认留言"
      @on-ok="addMessageFx"
    >
      <Input
        v-model="addMessageStr"
        :rows="8"
        type="textarea"
        placeholder="请在这里填写您的留言信息..."
      />
    </Modal>
    <Modal
      v-model="addReMessageModal"
      title="添加回复"
      ok-text="确认回复"
      @on-ok="addReMessageFx"
    >
      <Input
        v-model="addReMessageStr"
        :rows="8"
        type="textarea"
        placeholder="请在这里填写您的留言信息..."
      />
    ...
    >
      ...
      >
        <Page
          :current="researchForm.pageNumber"
          :total="retotal"
          :page-size="researchForm.pageSize"
          @on-change="changeRePage"
          @on-page-size-change="changeRePageSize"
          :page-size-opts="[15,20,50]"
          size="small"
          show-total
          show-elevator
...
export default {
  ...
  data () {
    return {
      lookReMessageModal: false,
      addReMessageStr: "",
      addReMessageModal: false,
      addMessageStr: "",
      ...
      reColumns: [
        {
          ...
          render: (h, params) => {
            return h(
              "span",
              params.index +
              (this.researchForm.pageNumber - 1) * this.researchForm.pageSize +
              1
            );
          },
        },
  ...
  methods: {
    ...
    replyFx (e) {
      this.panSelectRow = e;
      this.addReMessageModal = true;
    },
    dbClickPanRow (e) {
      this.panSelectRow = e;
      this.researchForm.replyId = e.id;
      this.getReDataList();
      this.lookReMessageModal = true;
    },
    addMessageFx () {
      var that = this;
      addMessageBoard({
        content: that.addMessageStr
      }).then(res => {
        if (res.success) {
    ...
    addReMessageFx () {
      var that = this;
      addReplyMessageBoard({
        content: that.addReMessageStr,
        replyId: that.zwzSelectRow.id
      }).then(res => {
        if (res.success) {
    ...
    getDataList () {
      this.loading = true;
      getMessageBoardList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
    ...
    getReDataList () {
      this.reLoading = true;
      getMessageBoardList(this.researchForm).then(res => {
        this.reLoading = false;
        if (res.success) {
          this.reData = res.result.records;
          this.retotal = res.result.total;
        }
      });
    },
    add () {
