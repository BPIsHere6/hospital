    ...
    >
      ...
      >
        <Row>
          <FormItem label="医生姓名">
            <Input
              v-model="panSelecrDoctor.doctorName"
              readonly
              style="width:570px"
            />
          </FormItem>
        ...
        <Row>
          <FormItem label="放号日期">
            <DatePicker
              :placeholder="panOrderDate"
              @on-change="changeAddNumberDate"
              clearable
              style="width: 570px"
            ></DatePicker>
          </FormItem>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            <Form-item label="">
              <DatePicker
                :placeholder="orderSearchForm.date"
                @on-change="changeAddNumberDate2"
                clearable
                style="width: 120px"
              ></DatePicker>
            </Form-item>
            ...
            >
              <Button
                @click="getDoctorSchedulingListFx"
                type="primary"
                icon="ios-search"
                size="small"
                ghost
      ...
      >
        <Page
          ...
          @on-page-size-change="changeOrderPageSize"
          :page-size-opts="[15,20,50]"
          size="small"
          show-total
          show-elevator
...
export default {
  ...
  methods: {
    ...
    addNumberFxFx () {
      var that = this;
      addNumber({
        doctorId: that.panSelecrDoctor.id,
        date: that.panOrderDate,
        step: that.panOrderStep,
        number: that.panOrderNumber
      }).then(res => {
        if (res.success) {
    ...
    addNumberFx (e) {
      this.panSelecrDoctor = e;
      this.panOrderDate = new Date().getFullYear() + "-" + ((new Date().getMonth() > 8) ? (new Date().getMonth() + 1) : "0" + (new Date().getMonth() + 1)) + "-" + (new Date().getDate() > 9 ? new Date().getDate() : "0" + new Date().getDate());
